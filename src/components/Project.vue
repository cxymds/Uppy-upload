	...
	<div class="system-user-container layout-padding">
		<el-card shadow="hover" class="layout-padding-auto">
			<div class="system-user-search mb15">
				<el-input size="default" placeholder="请输入项目名称" clearable style="max-width: 180px" v-model="state.tableData.param.project_name"> </el-input>
				...
				<el-button size="default" type="success" class="ml10" @click="onOpenProjectCreate()">
					<el-icon>
						<ele-FolderAdd />
					</el-icon>
					新增项目
			...
			<el-table :data="state.tableData.data" v-loading="state.tableData.loading" style="width: 100%">
				...
				<el-table-column label="操作" width="200">
					<template #default="scope">
						...
						@click="onOpenProjectUpload(scope.row)">上传</el-button
						>
						<el-button size="small" text type="primary" 
						@click="onOpenDetailUpload(scope.row)">查看</el-button
						>
...
const getTableData = async () => {
	...
	const res = await projectApi.projectList(state.tableData.param);
...
const onRowDel = (row: ProjectType) => {
	ElMessageBox.confirm(`此操作将永久删除文件：“${row.project_name}”，是否继续?`, '提示', {
		...
		.then(() => {
            projectApi.projectUploadDelete({project_id: row.id}).then(() => {
			    getTableData();
